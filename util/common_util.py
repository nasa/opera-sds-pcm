...
def create_expiration_time(latency):
    ...
    """
    return convert_datetime(datetime.datetime.now(datetime.UTC) + datetime.timedelta(minutes=latency))
...
def create_state_config_dataset(dataset_name, metadata, start_time, end_time=None, geojson=None,
    ...
    dataset_info = {
        "version": "1",
        pm.STATE_CONFIG_CREATION_TIME: convert_datetime(datetime.datetime.now(datetime.UTC)),
