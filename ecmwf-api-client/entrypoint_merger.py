...
def init_logging(level: Literal['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']):
    ...
    rfh1 = logging.handlers.RotatingFileHandler('merger.log', mode='a', maxBytes=100 * 2 ** 20, backupCount=10)
    rfh1.setLevel(logging.INFO)
    rfh1.setFormatter(logging.Formatter(fmt=log_file_format))
    logging.getLogger().addHandler(rfh1)

    rfh2 = logging.handlers.RotatingFileHandler('merger-error.log', mode='a', maxBytes=100 * 2 ** 20, backupCount=10)
...
if __name__ == '__main__':
    ...
    import merger
    merger.main(bucket_name=bucket_name, target_bucket_name=target_bucket_name, s3_keys=s3_keys)
