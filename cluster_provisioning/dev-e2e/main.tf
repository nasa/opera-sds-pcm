...
resource "null_resource" "mozart" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [<<-EOF
              set -ex
              source ~/.bash_profile
              ~/mozart/ops/opera-pcm/cluster_provisioning/purge_aws_resources.sh ${self.triggers.code_bucket}
              if [ "${var.clear_s3_aws_es}" = true ]; then
                python ~/mozart/ops/opera-pcm/cluster_provisioning/clear_grq_aws_es.py
                ~/mozart/ops/opera-pcm/cluster_provisioning/purge_aws_resources.sh ${self.triggers.dataset_bucket} ${self.triggers.triage_bucket} ${self.triggers.lts_bucket} ${self.triggers.osl_bucket}
              fi
    EOF
    ]
  }
