...
resource "null_resource" "mozart" {
  ...
  provisioner "remote-exec" {
    inline = [<<-EOF
              set -ex
              source ~/.bash_profile
              ~/mozart/ops/opera-pcm/cluster_provisioning/dev-e2e-pge-DISP_S1_hist/run_smoke_test.sh ~/mozart/ops/opera-pcm/cluster_provisioning/${module.common.smoke_test_config_file_name} || :
    EOF
    ]
  }
