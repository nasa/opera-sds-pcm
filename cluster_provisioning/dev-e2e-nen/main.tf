...
resource "null_resource" "mozart" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sds -d ship",
      "~/mozart/ops/opera-pcm/cluster_provisioning/run_smoke_test_nen.sh \\",
      "  ${var.project} \\",
      "  ${var.environment} \\",
      "  ${var.venue} \\",
  ...
  provisioner "remote-exec" {
    inline = [
      "set -ex",
      "source ~/.bash_profile",
      "~/mozart/ops/opera-pcm/conf/sds/files/test/dump_failed_jobs.py",
    ]
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "set -ex",
      "source ~/.bash_profile",
      "pytest ~/mozart/ops/opera-pcm/cluster_provisioning/dev-e2e-nen/check_pcm.py ||:",
    ]
  }
