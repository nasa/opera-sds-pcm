...
resource "null_resource" "destroy_es_snapshots" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "  # Purging is for development venues only. An ops venue reaching this branch means the",
      "  # destroy action was misconfigured, and the archive it would delete is not reproducible.",
      "  case \"${self.triggers.venue}\" in ops*) echo Refusing to purge ES snapshots for an ops venue; exit 1;; esac",
      ...
      "    # Both the current pair of policies and the single policy older clusters were built",
      "    # with; a missing one is not an error worth aborting a destroy over.",
      "    ~/mozart/bin/snapshot_es_data.py --es-url ${self.triggers.grq_es_url} delete-lifecycle --policy-id hourly-snapshot-grq || true",
      "    ~/mozart/bin/snapshot_es_data.py --es-url ${self.triggers.grq_es_url} delete-lifecycle --policy-id hourly-snapshot-mozart-metrics || true",
      "    ~/mozart/bin/snapshot_es_data.py --es-url ${self.triggers.grq_es_url} delete-lifecycle --policy-id hourly-snapshot || true",
      ...
      "  ~/mozart/bin/snapshot_es_data.py --es-url ${self.triggers.grq_es_url} create-snapshot --repository snapshot-repo --snapshot ${lower("${self.triggers.project}-${self.triggers.venue}-${self.triggers.counter}_teardown_snapshot_${timestamp()}")} --wait --index-pattern \"*,-.*,-jobs_accountability_catalog*\"",
