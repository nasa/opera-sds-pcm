...
resource "aws_lambda_function" "slcs1c_query_timer" {
  depends_on    = [null_resource.download_lambdas]
  filename      = "${var.lambda_data-subscriber-query_handler_package_name}-${var.lambda_package_release}.zip"
  description   = "Lambda function to submit a job that will query Sentinel SLC 1C data."
  ...
  environment {
    variables = {
      ...
      "JOB_RELEASE" : var.pcm_branch,
      "MINUTES" : var.slcs1c_query_timer_trigger_frequency,
      "PROVIDER" : var.slc_provider,
      "ENDPOINT" : "OPS",
      "DOWNLOAD_JOB_QUEUE" : var.queues.opera-job_worker-slc_data_download.name,
      "CHUNK_SIZE" : "1",
...
resource "aws_cloudwatch_log_group" "slcs1c_query_timer" {
  name              = "/aws/lambda/${aws_lambda_function.slcs1c_query_timer.function_name}"
  retention_in_days = var.lambda_log_retention_in_days
}
...
resource "aws_cloudwatch_event_target" "slcs1c_query_timer" {
  rule       = aws_cloudwatch_event_rule.slcs1c_query_timer.name
  target_id  = "Lambda"
  arn        = aws_lambda_function.slcs1c_query_timer.arn
  depends_on = [null_resource.setup_trigger_rules]
}
resource "aws_lambda_permission" "slcs1c_query_timer" {
  statement_id  = aws_cloudwatch_event_rule.slcs1c_query_timer.name
  action        = "lambda:InvokeFunction"
  principal     = "events.amazonaws.com"
...
resource "aws_lambda_function" "rtc_for_dist_query_timer" {
  depends_on    = [null_resource.download_lambdas]
  filename      = "${var.lambda_data-subscriber-query_handler_package_name}-${var.lambda_package_release}.zip"
  description   = "Lambda function to submit a job that will query RTC data for DIST-S1 production."
  ...
  environment {
    variables = {
      ...
      "JOB_RELEASE" : var.pcm_branch,
      "MINUTES" : var.rtc_for_dist_query_timer_trigger_frequency,
      "PROVIDER" : var.rtc_provider,
      "ENDPOINT" : "OPS",
...
resource "aws_cloudwatch_log_group" "rtc_for_dist_query_timer" {
  name              = "/aws/lambda/${aws_lambda_function.rtc_for_dist_query_timer.function_name}"
  retention_in_days = var.lambda_log_retention_in_days
}
...
resource "aws_cloudwatch_event_target" "rtc_for_dist_query_timer" {
  rule       = aws_cloudwatch_event_rule.rtc_for_dist_query_timer.name
  target_id  = "Lambda"
  arn        = aws_lambda_function.rtc_for_dist_query_timer.arn
  depends_on = [null_resource.setup_trigger_rules]
}
resource "aws_lambda_permission" "rtc_for_dist_query_timer" {
  statement_id  = aws_cloudwatch_event_rule.rtc_for_dist_query_timer.name
  action        = "lambda:InvokeFunction"
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.rtc_for_dist_query_timer.arn
