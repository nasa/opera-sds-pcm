    ...
    inline = [<<-EOT
     while [ ! -f /var/lib/cloud/instance/boot-finished ]; do echo 'Waiting for cloud-init...'; sleep 10; done
      ...
      if [ "${local.es_cluster_mode}" = false ]; then
        echo // grq
        curl -k --netrc-file ~/.netrc-os -XPUT ${local.grq_es_url}/_cluster/settings -H 'Content-type: application/json' --data-binary $'{"transient":{"cluster.max_shards_per_node": ${var.max_shards_per_node}, "search.max_open_scroll_context": 6000}, "persistent":{"cluster.max_shards_per_node": ${var.max_shards_per_node}, "search.max_open_scroll_context": 6000}}'
