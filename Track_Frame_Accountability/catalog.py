...
REFREC_ID = "refrec_id"
ES_INDEX = "track_frame_accountability_catalog"
ES_TYPE = "track_frame"
HEADER = "header"


class TrackFrameAccountabilityCatalog(ElasticsearchUtility):
    """
    ...
    def __add_mapping(self, index, mapping_type):
        ...
        mappings_file = os.path.join(
            current_directory, "es_mapping", mappings_file)
    ...
    def create_index(self, index=ES_INDEX, mapping_type=ES_TYPE, delete_old_index=False):
        ...
        if self.logger:
            self.logger.info(
                "Successfully add mapping to index {}".format(index))
