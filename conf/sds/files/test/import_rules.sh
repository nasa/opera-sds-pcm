...
LW_VERSION=$(curl -k --netrc-file ~/.netrc-os -s -XGET $MOZART_ES_URL/containers/_doc/${LW_PKG}?_source=version | python -c "import json,sys;obj=json.load(sys.stdin);print(obj['_source']['version']);")
