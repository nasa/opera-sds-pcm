...
# The tool reaches the cluster's ES connection at module scope; mock it before importing.
for _cluster_module in ("hysds", "hysds.celery", "hysds_commons", "hysds_commons.elasticsearch_utils",
    ...
    sys.modules.setdefault(_cluster_module, umock.MagicMock())
...
def test_generate_initial_frame_states():
    result = generate_initial_frame_states([[18904, 18905], 16669])
    print(result)
    assert result == {18904: 0, 18905: 0, 16669: 0}
...
def test_form_job_params_basic():
    ...
    do_submit, job_name, job_spec, job_params, job_tags, next_frame_sensing_position, finished = \
        form_job_params(p, 16669, 0, None, None)
    ...
    assert "data-subscriber-query-timer-historical1_f16669" in job_name
    assert "2016-08-23T23:40:" in job_name
    assert "2016-10-23T00:40:" in job_name
    assert JOB_TYPE in job_spec
    assert job_tags == ['data-subscriber-query-timer', 'historical_processing']
    assert "--start-date=2016-08-23T23:40:" in job_params["start_datetime"]
    assert "--end-date=2016-10-23T00:40:" in job_params["end_datetime"]
    ...
    assert job_params["frame_id"] == f'--frame-id=16669'
