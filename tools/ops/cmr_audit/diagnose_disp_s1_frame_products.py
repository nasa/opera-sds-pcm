...
def diagnose_frame(frame_id, start_date, end_date, k=15, show_last_n=20):
    ...
    # Show products outside historical database if any
    if invalid_products:
        print("=" * 120)
        print("PRODUCTS OUTSIDE HISTORICAL DATABASE")
        print("=" * 120)

        # Get first and last sensing times from frame info
        first_sensing = frame.sensing_datetimes[0]
        last_sensing = frame.sensing_datetimes[-1]

        # Categorize by whether they're before or after the historical range
        before_range = []
        after_range = []
        for p in invalid_products:
            try:
                end_dt = dateutil.parser.isoparse(p['end_date']).replace(tzinfo=None)
                if end_dt < first_sensing.replace(tzinfo=None):
                    before_range.append(p)
                else:
                    after_range.append(p)
            except:
                after_range.append(p)  # Default to after if parsing fails

        print(f"Historical database range: {first_sensing.strftime('%Y-%m-%d')} to {last_sensing.strftime('%Y-%m-%d')}")
        print(f"Products before range: {len(before_range)}")
        print(f"Products after range (forward processing): {len(after_range)}")
        print()

        def extract_production_date(granule_ur):
            """Extract production date from granule UR (last date in filename)"""
            try:
                # Pattern: ..._YYYYMMDDTHHMMSSZ at the end before version
                parts = granule_ur.split('_')
                for part in reversed(parts):
                    if part.startswith('20') and len(part) >= 15 and 'T' in part:
                        return part[:15]  # Return YYYYMMDDTHHMMSS
                return "???"
            except:
                return "???"

        if before_range:
            print("Products BEFORE historical range (may be from different processing or data issue):")
            print(f"  {'End Date':>25} | {'Production Date':>17} | Product ID")
            print(f"  {'-' * 110}")
            sorted_before = sorted(before_range, key=lambda x: x['end_date'])
            for p in sorted_before[:10]:
                prod_date = extract_production_date(p['granule_ur'])
                print(f"  {p['end_date'][:25]:>25} | {prod_date:>17} | {p['granule_ur'][:60]}")
            if len(sorted_before) > 10:
                print(f"    ... and {len(sorted_before) - 10} more")
            print()

        if after_range:
            print("Products AFTER historical range (forward/real-time processing):")
            print(f"  {'End Date':>25} | {'Production Date':>17} | Product ID")
            print(f"  {'-' * 110}")
            sorted_after = sorted(after_range, key=lambda x: x['end_date'])
            for p in sorted_after[:10]:
                prod_date = extract_production_date(p['granule_ur'])
                print(f"  {p['end_date'][:25]:>25} | {prod_date:>17} | {p['granule_ur'][:60]}")
            if len(sorted_after) > 10:
                print(f"    ... and {len(sorted_after) - 10} more")
            print()
