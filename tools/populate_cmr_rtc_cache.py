...
def populate_cmr_rtc_cache(granules: List[Dict[str, Any]], es_conn) -> None:
    ...
    with logging_redirect_tqdm():
        ...
        with concurrent.futures.ThreadPoolExecutor(concurrency) as executor:
            ...
            # results = [future.result() for future in concurrent.futures.as_completed(tqdm(futures))]
            concurrent.futures.wait(tqdm(futures))
    # Refresh index
...
def main():
    ...
    if args.db_file:
        ...
        dist_products, bursts_to_products, product_to_bursts, all_tile_ids = parse_local_burst_db_pickle(args.db_file,
                                                                                                         pickle_file_name)
    else:
