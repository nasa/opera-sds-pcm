...
def determine_projection(poly, track_frame):
    ...
    xx, yy = np.meshgrid(np.linspace(minX, maxX, 250),
                         np.linspace(minY, maxY, 250))
    x = xx.flatten()
    y = yy.flatten()
...
def download_dem(poly, epsg, margin, outfile):
    ...
    print("S3 bucket DEM filepath: {}".format(vrt_filename))
...
def return_dem_filepath(epsg, ref_slc=None,
    ...
    vrt_filename = '/vsis3/opera-dem/EPSG' + str(epsg) + \
                   '/EPSG' + str(epsg) + '.vrt'
...
def check_aws_connection():
    """
       Check if user can read files from s3://opera-dem
       and throw an exception if not possible
    """
    import boto3

    s3 = boto3.resource('s3')
    obj = s3.Object('opera-dem', 'EPSG32610/EPSG32610.vrt')
    ...
    try:
        ...
        errmsg = 'No access to opera-dem s3 bucket. Check your AWS credentials' \
                 'and re-run the code : {}'.format(str(e))
...
def main(opts):
    ...
    if os.path.isfile(opts.filepath):
        ...
        return os.path.join(os.getcwd(), opts.outfile)
